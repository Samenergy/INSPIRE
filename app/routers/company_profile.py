"""Company Profile Generation Router - Scrape and Extract Company Intelligence.

This router provides a simple endpoint to generate company profiles by scraping
articles and extracting key information.
"""
...
def get_intelligence_service():
    """Lazy initialize intelligence service."""
...
def get_profile_aggregator():
    """Lazy initialize profile aggregator."""
...
async def generate_company_profile(
    ...
    """
    Generate comprehensive company profile from scraped articles.
    
    **Input:**
    - company_name: Name of the company
    - location: Country/region where company operates
    - max_articles: How many articles to analyze (more = better quality, slower)
    
    **Output:**
    - description: What the company does (1-2 sentences)
    - strengths: List of company strengths (up to 10)
    - weaknesses: List of company weaknesses (up to 8)
    ...
    **Example:**
    ...
    Returns:
    {
      "description": "MTN Rwanda is the leading telecommunications provider...",
      "strengths": [
        "Market Leadership: 62.9% market share...",
        "Customer Base: 7.6 million subscribers..."
      ],
      "weaknesses": [
    ...
    try:
        ...
        # Step 1: Scrape news articles using SerpAPI
        ...
        # Step 2: Fetch full article content from URLs
        ...
        try:
            ...
            enhanced_articles = articles_to_enhance  # Fallback to snippets
        finally:
            await content_fetcher.close()
        
        # Step 3: Extract intelligence (description, strengths, weaknesses) from articles  
        logger.info("Step 3/4: Extracting intelligence from articles...")
        
        # Extract intelligence from each article
        intelligence_service = get_intelligence_service()
        all_strengths = []
        all_weaknesses = []
        ...
        for article in enhanced_articles:
            intelligence = intelligence_service.extract_intelligence(
                ...
                company_name=company_name  # Pass company name for better description validation
            )
            all_strengths.extend(intelligence['strengths'])
            all_weaknesses.extend(intelligence['weaknesses'])
            
            # Collect descriptions as fallback
        ...
        # PRIORITIZE: Get overview from SerpAPI "what is..." query FIRST
        ...
        # Extract strengths from overview text (Knowledge Graph, Wikipedia, etc.)
        overview_strengths = []
        ...
        # Combine strengths from overview AND articles
        ...
        # Format the response - ONLY description, strengths, weaknesses
        response_data = {
...
async def generate_formatted_profile(
    ...
    """
    Generate company profile with formatted text output.
    
    Returns both structured data AND a formatted text version ready for display.
    """
    try:
        # Get the profile using the main endpoint
...
async def get_service_info():
    """
    Get information about the company profile generation service.

"""Comprehensive scraping router for unified company data collection."""
...
async def comprehensive_scrape(
    ...
    """
    Perform comprehensive company data scraping.
    
    **Workflow:**
    1. **Company Creation**: Creates or updates company record
    2. **SerpAPI Scraping**: Searches for company info, news, LinkedIn URL
    3. **Apify LinkedIn Scraping**: Scrapes LinkedIn posts if URL found
    4. **Data Aggregation**: Returns all collected data
    
    **Returns:**
    - Company information
    - News articles from SerpAPI
    - LinkedIn posts from Apify
    - Website updates
...
async def get_scraping_status():
    """Get the current status of the comprehensive scraping service."""
    return APIResponse(
        success=True,
        ...
        data={
            ...
            "status": "operational",
            "available_sources": ["serpapi"],  # Apify temporarily disabled
            "workflow": [
...
async def get_csv_exports(company_name: str):
    """Get information about CSV files exported for a specific company."""
    try:

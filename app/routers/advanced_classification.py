"""Advanced classification router for ML-based article classification."""
...
async def classify_articles_upload(
    ...
    """
    Classify articles from uploaded CSV file.
    
    **Request:**
    - **file**: CSV file with articles (required columns: title, content)
    - **company_objective**: Business objective for relevance classification
    - **use_custom_objective**: Enable weak supervision with custom objective
    
    **Returns:**
    - Classification results for each article
    - Confidence scores and probabilities
    - Summary statistics
    - Model information
    """
...
async def classify_single_article(
    ...
    **Returns:**
    ...
    - Weak supervision results (if enabled)
    """
    try:

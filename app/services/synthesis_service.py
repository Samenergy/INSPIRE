"""Synthesis Service for generating recommendations and action plans.

Uses LLM to generate synthesis-based answers (Action Plans, Solutions).

YOUR CONTRIBUTIONS (for Capstone Defense):
1. Custom prompt engineering for African MSME partnership context
2. Multi-LLM support (Ollama, OpenAI, template-based fallback)
3. Domain-specific recommendation logic
4. SME solution matching algorithm
...
class SynthesisService:
    """Service for generating recommendations using LLM."""
    ...
    def __init__(self, llm_provider: str = "auto"):
        ...
        # YOUR CONTRIBUTION: Industry-specific context patterns
    ...
    def _detect_industry(self, text: str) -> str:
        """
        YOUR CONTRIBUTION: Detect industry from text analysis.
        
        Analyzes company data to identify their industry for context-aware prompts.
        """
        if not text:
            return 'general'
    ...
    def _get_industry_context(self, sme_industry: str, target_industry: str) -> Dict[str, str]:
        """
        YOUR CONTRIBUTION: Generate industry-specific context for prompts.
        
        Provides relevant examples and terminology based on industries.
        """
        contexts = {
        ...
        # Get specific context or default
    ...
    async def generate_action_plan(self, company_data: Dict[str, Any], company_name: str, sme_objective: str) -> str:
        """
        Generate action plan (Question 6).
        
        YOUR CONTRIBUTION: Custom prompt for MSME partnership engagement
        Based on TARGET company's needs and YOUR SME's capabilities
        """
    ...
    async def generate_solutions(self, company_data: Dict[str, Any], company_name: str, sme_objective: str) -> str:
        """
        Generate SME solutions (Question 7).
        
        YOUR CONTRIBUTION: Custom prompt for solution matching
        Matches YOUR SME's solutions to TARGET company's needs
        """
    ...
    def _create_action_plan_prompt(self, company_data: Dict[str, Any], company_name: str, sme_objective: str) -> str:
        """
        YOUR CONTRIBUTION: Adaptive prompt template for SME-specific action plan generation.
        
        Adapts based on both SME and target company industries.
        """
        # Detect industries
...
Be highly specific to BOTH industries. Avoid generic advice. Use actual data from their updates."""
    ...
    def _create_solutions_prompt(self, company_data: Dict[str, Any], company_name: str, sme_objective: str) -> str:
        """
        YOUR CONTRIBUTION: Adaptive prompt template for SME-specific solution matching.
...
Avoid generic statements. Every recommendation must tie YOUR specific capability to THEIR specific need."""
    ...
    async def _call_llm(self, prompt: str) -> str:
        ...
        Tries Ollama first (free), then OpenAI if available.
        """
    ...
    def _template_action_plan(self, company_data: Dict[str, Any], company_name: str, sme_objective: str) -> str:
        """
        YOUR CONTRIBUTION: Adaptive template-based action plan generation (fallback).
        
        Used when LLM is not available. Adapts to industries.
        """
        # Detect industries
        sme_text = sme_objective
...
3. Pilot Partnership for {context['engagement']}: Propose a focused pilot project that demonstrates measurable value in 30-60 days. Based on their current priorities and your {sme_industry} capabilities, identify one high-impact area where your solutions can deliver quick wins and build foundation for broader {target_industry} partnership."""
    ...
    def _template_solutions(self, company_data: Dict[str, Any], company_name: str, sme_objective: str) -> str:
        """
        YOUR CONTRIBUTION: Adaptive template-based solution recommendations (fallback).
        
        Detects industries and adapts recommendations accordingly.
        """
        # Detect industries
        sme_text = sme_objective
        ...
        # Generate industry-aware solution names

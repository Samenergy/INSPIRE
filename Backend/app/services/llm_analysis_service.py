...
class LLMAnalysisService:
    ...
    def __init__(self, llm_provider: str = "auto"):
        ...
        # Try OLLAMA_BASE_URL first, fallback to OLLAMA_URL or default
        self.ollama_url = os.getenv('OLLAMA_BASE_URL') or os.getenv('OLLAMA_URL', 'http://localhost:11434')

        self.industry_patterns = {
            'fintech': ['fintech', 'payment', 'mobile payment', 'digital payment', 'mobile money', 'momo', 'wallet', 'transaction', 'financial services', 'lending', 'merchant', 'pos', 'payment gateway', 'payment solution', 'payment platform', 'payment processing', 'merchant management', 'business analytics'],
            'telecommunications': ['telecom', 'mobile', 'network', 'connectivity', '5g', '4g', 'subscriber', 'mtn', 'airtel', 'vodafone', 'mobile operator'],
            'banking': ['bank', 'banking', 'lending', 'credit', 'loan', 'financial institution', 'microfinance'],
            ...
            'technology': ['technology', 'software', 'platform', 'digital', 'cloud', 'saas', 'tech', 'app', 'it solutions']
        }
    ...
    def _prepare_articles_context(self, articles: List[Dict[str, str]]) -> str:
        ...
        substantial_articles = [a for a in articles if len(a.get('content', '')) > 150]

        articles_to_use = substantial_articles if len(substantial_articles) >= 5 else articles

        articles_to_use = articles_to_use[:12]

        logger.info(f"📄 Preparing context: {len(articles_to_use)} articles (filtered from {len(articles)})")

        context_parts = []

        for i, article in enumerate(articles_to_use, 1):
